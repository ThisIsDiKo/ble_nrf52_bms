...
BT_GATT_SERVICE_DEFINE(remote_srv,
    BT_GATT_PRIMARY_SERVICE(BT_UUID_REMOTE_SERVICE),
        ...
        BT_GATT_CCC(button_chrc_ccc_cfg_changed, BT_GATT_PERM_READ_ENCRYPT | BT_GATT_PERM_WRITE_ENCRYPT),
			  ...
			  uint8_t flags)
{
    ...
    if (remote_callbacks.data_received){
        printk("Process %d bytes to callback", len);
        remote_callbacks.data_received(conn, buf, len);
    }
    else{
        printk("No callback to pass to");
    }
    return len;
}
...
{
    int err;
    printk("Initializing bluetooth callbacks...");
